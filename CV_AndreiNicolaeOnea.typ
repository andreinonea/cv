...
#cv.jobrole(
  ...
  Teamed with two seasoned programmers and built from scratch a web server in _Rust_, using the _actix_ framework, to control the various functions of the hardware.
  Contributed to complex solutions such as a flexible and easy-to-use object model for dynamic endpoint de/registration with built-in attribute monitoring, a job system with support for both OS processes and internal function calls, a "driver" model with runtime loading inspired by the _Linux_ kernel.

  Implemented software solutions needed for production hardware stress-testing and qualification.
  Integrated and configured a custom _Linux_ kernel for embedded devices using _Yocto_, along with other open-source and proprietary packages.
  ...
  As a result, areas such as the following have been positively impacted: source code and workflow optimization, code reviews and verification, traceability in decision-making, tests results and final artifacts.
]
...
#cv.degree(
  ...
  Thesis: Emotion-Driven Characters: A Study on Emergent NPC Behavior
  ...
  Goal is to enable emotion-driven characters for greater replayability and entertainment.

  Simulation runs on a custom engine developed in parallel, made with _Rust_ and _OpenGL_.
]
...
#cv.project(name: [Blockytry])[
  This is an _OpenGL_ playground to grow my graphics programming skills.
...
#cv.project(name: ["Fireworks filmed with a drone"])[
  ...
  Explored _OpenGL_ programmable pipeline by writing from scratch all necessary code to output "fireworks" on a starry night background (cubemap), and some extra text rendering for debugging.
]
...
#cv.interests(items: (
  (
    topic: [Technology],
    description: [Emergent behavior in NPCs, physics simulations and real-time rendering, telesurgery],
  ),
  (
    topic: [Education],
    description: [Mentoring, teaching mathematics, physics and computer graphics in an intuitive way],
  ),
  (
    topic: [Hobbies],
  ...
  (
    topic: [Learning],
    description: [
      Greek, Korean, Arabic, Persian, Spanish and German sporadically
    ],
  ),
  (
    topic: [Sports],
